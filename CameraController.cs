...
public class CameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(HeadEulerRotation.x) > HeadRotationLimit.y) 
        { 
            HeadEulerRotation.x = HeadRotationLimit.y * Mathf.Sign(HeadEulerRotation.x);
        }

        // Move body when head passes rotation limit.
        if (Mathf.Abs(HeadEulerRotation.y - BodyEulerRotation.y) > HeadRotationLimit.x)
        {
            BodyEulerRotation += new Vector3(0, MouseVelocity.x, 0) * CameraSensitivity; // Update the rotation vector of the body.
        }
